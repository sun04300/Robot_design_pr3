"""
[파일] Camera_v6.py
[목적] Camera_v4 + Ki+LiDAR_Opt 장점 합성
  · Camera_v4  : VFH(OBS_RET+측면보정), CLOSE-FWD 히스테리시스, INVIS 카운터, MEM 2s
  · Ki+LiDAR_Opt: solvePnP 거리 기반 정밀 정지(ENTRY_STOP_MM=250mm), ALIGN 회전 모드

[제어 흐름]
  ① 강탐지 → PnP 시도
        PnP 성공: z_mm 기반 속도 제어 → z < ENTRY_STOP_MM 즉시 STOP
                  멀리서 오정렬 → ALIGN (T 명령 제자리 회전)
        PnP 실패: area_r 기반 CLOSE-FWD / FWD-NQ / CURVE (폴백)
  ② area_peak_seen 후 미탐지 → ENTERING / INVIS 카운터 → STOP
  ③ 미탐지 → VFH(OBS_RET) 탐색
"""
...
CONFIRM_FRAMES    = 10
STOP_DURATION     = 1.1
COLOR_MEMORY_TIME = 2.00
STEER_SMOOTH_ALPHA = 0.45

# ─────────────────────────────────────────────────────────────────────────────
#  PnP 정밀 정지 / ALIGN 파라미터
# ─────────────────────────────────────────────────────────────────────────────
STEER_GAIN        = 0.015   # PnP 각도(도) → steer 변환 계수
ENTRY_STOP_MM     = 250.0   # PnP 거리 이하 즉시 STOP
DIST_SLOW_MM      = 500.0   # 이 거리 이내부터 SPEED_NEAR 사용
ALIGN_STEER_ENTER = 0.30    # ALIGN 진입 임계값 (|steer| 초과)
ALIGN_STEER_EXIT  = 0.12    # ALIGN 해제 임계값 (|steer| 이하)
ALIGN_MIN_DIST    = 450.0   # ALIGN 활성화 최소 거리 (mm), 근거리는 바로 접근
ALIGN_ROT_SPEED   = 0.30    # ALIGN 제자리 회전 속도
...
# ─────────────────────────────────────────────────────────────────────────────
#  LiDAR VFH 헬퍼 (Camera_v4 동일 — OBS_RET 측면보정 포함)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
#  solvePnP 기반 종이 거리/각도 추출 (Ki+LiDAR_Opt)
# ─────────────────────────────────────────────────────────────────────────────
...
def solve_paper_pose(contour, cam_mat, dist_coeffs):
    """컨투어 → solvePnP → (z_mm, x_mm, steer, quad_pts) 또는 None."""
    if cam_mat is None:
        return None
...
def main():
    ...
    # 캘리브레이션 로드 → undistort 맵 + PnP 행렬 준비
    ...
    if cam_mat is not None and calib_res == (CAM_W, CAM_H):
        ...
        pnp_mat  = new_mtx           # undistort 후 왜곡 없음
        pnp_dist = np.zeros((4, 1), dtype=np.float64)
        print("[CAM] 캘리브레이션 적용 + PnP 활성화")
    elif cam_mat is not None:
        pnp_mat  = cam_mat
        pnp_dist = dist_coeffs if dist_coeffs is not None else pnp_dist
        print("[CAM] 캘리브레이션 적용 (undistort 없음) + PnP 활성화")
    else:
        print("[CAM] 캘리브레이션 없음 → PnP 비활성화, area_r 폴백 사용")
    ...
    print("=" * 60)
    print("  Camera_v6  |  PnP정밀정지 + ALIGN + VFH OBS_RET")
    print(f"  처리해상도: {CAM_W}×{CAM_H}   목표: RED→YELLOW→BLUE")
    print(f"  PnP: {'활성' if pnp_mat is not None else '비활성(area_r 폴백)'}")
    print("=" * 60)
    ...
    while True:
        ...
        if det is not None:
            ...
            # area_peak_seen 갱신 (PnP 실패 시 INVIS 폴백용)
            if area_r >= AREA_PEAK_THRES:
                sv['area_peak_seen'] = True
                sv['peak_area_r']    = max(sv['peak_area_r'], area_r)

            # ── PnP 시도 ─────────────────────────────────────────────────
            pose = solve_paper_pose(det['contour'], pnp_mat, pnp_dist)

            if pose is not None:
                z_mm, x_mm, pnp_steer, quad_pts = pose

                # 근접 시 폴백 보험: area_peak_seen 강제 설정
                if z_mm < 600:
                    sv['area_peak_seen'] = True
                ...
                if z_mm < ENTRY_STOP_MM:
                    ...
                    ser.write(b"S\n")
                    print(f"  🎯 {color.upper()} PnP 도달! z={z_mm:.0f}mm")
                    cv2.imshow('Robot View', vis)
                    cv2.waitKey(1)
                    continue

                # ── ALIGN: 멀리서 오정렬 → 제자리 회전 ──────────────
                if abs(pnp_steer) > ALIGN_STEER_ENTER and z_mm > ALIGN_MIN_DIST:
                    sv['align_mode'] = True
                elif abs(pnp_steer) < ALIGN_STEER_EXIT:
                    sv['align_mode'] = False

                if sv['align_mode']:
                    rot_sign = ALIGN_ROT_SPEED if pnp_steer > 0 else -ALIGN_ROT_SPEED
                    ser.write(f"T {rot_sign:.2f}\n".encode())
                    cv2.polylines(vis, [quad_pts.astype(np.int32)], True, (0, 200, 255), 2)
                    cv2.putText(vis, f"ALIGN z={z_mm:.0f}mm st={pnp_steer:+.2f}",
                                (CAM_W // 2 - 170, 44),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 200, 255), 2)
                    print(f"  [ALIGN] {color.upper()} z={z_mm:.0f}mm pnp_st={pnp_steer:+.2f}")
                    sv['prev_area_r'] = area_r
                    sv['approach_steer_locked'] = False
                    cv2.imshow('Robot View', vis)
                    cv2.waitKey(1)
                    continue
                ...
                _draw_ctr(vis, int(ctr[0]), int(ctr[1]), (0, 220, 255))
                cv2.putText(vis, f"PnP z={z_mm:.0f}mm x={x_mm:+.0f}mm A={area_r:.2f}",
                            (CAM_W // 2 - 190, 44),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 220, 255), 2)

                if speed > 0:
                    ser.write(f"F {smoothed_steer:.2f} {speed:.2f}\n".encode())
                    print(f"  [PnP] {color.upper()} z={z_mm:.0f}mm st={smoothed_steer:+.2f} spd={speed:.2f}")
                else:
                    vfh_log = _vfh_drive(ser)
                    print(f"  [PnP→VFH] {color.upper()} obstacle→ {vfh_log}")

            else:
                # ── PnP 실패 → area_r 기반 폴백 (Camera_v4 로직) ─────
                if area_r >= AREA_SLOW_THRES or (
                        sv['approach_steer_locked'] and sv['area_peak_seen'] and area_r >= 0.15):
                    if not sv['approach_steer_locked']:
                        if sv['prev_area_r'] > 0.05:
                            sv['locked_approach_steer'] = float(
                                np.clip(offset * 0.25, -0.35, 0.35))
                        else:
                            sv['locked_approach_steer'] = 0.0
                        sv['approach_steer_locked'] = True
                    steer   = sv['locked_approach_steer']
                    speed   = _speed_limit(SPEED_NEAR)
                    src     = "approach" if sv['prev_area_r'] > 0.05 else "sudden"
        ...
        # ② 피크 후 미탐지 → ENTERING / INVIS ──────────────────────────
        elif sv['area_peak_seen']:
            sv['prev_area_r'] = 0.0
            ...
            if w is not None:
                ...
                cv2.putText(vis,
                            f"INVISIBLE cnt:{sv['on_zone_count']}/{CONFIRM_FRAMES} nudge={nudge_spd:.2f}",
                            (10, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 255, 180), 2)
                print(f"  [INVIS] {color.upper()} pk={sv['peak_area_r']:.2f}"
                      f" cnt={sv['on_zone_count']} nudge={nudge_spd:.2f}")
                if sv['on_zone_count'] >= CONFIRM_FRAMES:
                    state      = 'STOP'
                    stop_start = time.time()
                    ser.write(b"S\n")
                    print(f"  🎯 {color.upper()} 도달! peak={sv['peak_area_r']:.2f}")
                    cv2.imshow('Robot View', vis)
                    cv2.waitKey(1)
                    continue

        # ③ 미탐지 → VFH 탐색 (OBS_RET 포함) ───────────────────────────
        else:
            sv['prev_area_r']   = 0.0
            sv['on_zone_count'] = max(0, sv['on_zone_count'] - 1)
            w = _weak_detect(undist, color)
            if w is not None:
                cnt_w, wcx, wcy = w
                weak_offset = _offset(wcx)
                ...
                if elapsed < COLOR_MEMORY_TIME:
                    ...
                    if obstacle_now:
                        sv['last_obs_steer'] = float(np.clip(
                            (ls['lat_L'] - ls['lat_R']) / (DETECT + 1e-9), -1.0, 1.0))
                        if not sv['obs_active']:
                            sv['obs_clear_time'] = None
                        sv['obs_active'] = True
                    elif sv['obs_active']:
                        sv['obs_active']     = False
                        sv['obs_clear_time'] = time.time()

                    return_elapsed = (time.time() - sv['obs_clear_time']) \
                        if sv['obs_clear_time'] is not None else OBS_RETURN_TIME + 1.0

                    if return_elapsed < OBS_RETURN_TIME:
                        ret_st  = float(np.clip(
                            sv['last_obs_steer'] * OBS_RETURN_GAIN, -MAX_STEER, MAX_STEER))
        ...
        cv2.putText(vis,
                    f"{state}|{color.upper()}|cnt:{sv['on_zone_count']}/{CONFIRM_FRAMES}{emg_txt}",
