...
# YELLOW: 카메라 실측값 기반 (2026-06-21 경기장 실측)
#  카메라 측정값: 노란종이 R=252,G=252,B=218 → HSV(H=30, S=34, V=252)
#                코부키박스 R=188,G=168,B=132 → HSV(H=19, S=76, V=188)
#
#  [중요] S값이 역전됨: 종이(S=34) < 박스(S=76) → S_min으로 박스 차단 불가
#  H로 구분: 종이 H=30 vs 박스 H=19 → H_min=23으로 박스 제외
#  V_min=140: 근거리 과노출 V=252 기준이나 원거리에서 낮아질 수 있음 → 여유분 확보
#  박스(H=19)는 H_min=23에서 차단되므로 V 기준 완화해도 오탐 없음
#  S_min=10: 종이가 S=34로 매우 낮으므로 하한선 낮게 설정
YELLOW_LOWER = np.array([ 23,  10, 140])
